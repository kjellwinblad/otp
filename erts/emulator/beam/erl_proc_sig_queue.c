...
/*     while (sig = *first; sig; sig = sig->next) { */
/*         ERTS_ASSERT(e) */
/*         if (ERTS_SIG_IS_NON_MSG(sig)) */
/*             ++cnt; */
/*     } */
                ...
                int* erts_psflg_sig_in_q_updated_wb)
{
    ...
    *this = first;
    ...
    no_nmsig:
        ;/* TODO is it fine that this assertion no longer holds? */
...
{
    int dummy;
    return enqueue_signals(rp, first, last, last_next, num_msgs, in_state, in_state & ERTS_PSFLG_OFF_HEAP_MSGQ, 0, &rp->sig_inq, &dummy);
}
...
{
    int dummy;
    return enqueue_signals(rp, first, last, last_next, num_msgs, in_state, flush_buffers, 0, &rp->sig_inq, &dummy);
}
...
first_last_done:
    ...
    if (ERTS_PSFLG_FREE & state)
        ...
        int dummy;
        state = enqueue_signals(rp, first, &last->next, last_next, 0, state, 1, 0, &rp->sig_inq, &dummy);
...
erts_proc_sig_fetch__(Process *proc)
{
    ...
    if (!proc->sig_inq.nmsigs.next) {
        // We can get a new signal to a buffer at any time
         ...
         if (!proc->sig_qs.nmsigs.last) {
            ...
            s = erts_atomic32_read_bset_nob(&proc->state,
                                            (ERTS_PSFLG_SIG_Q
                                             | ERTS_PSFLG_SIG_IN_Q),
                                            ERTS_PSFLG_SIG_Q);
            // This can happen beacuse we might get here early because
            //buffered messages set this flag before taking the lock
                          ...
                          Eterm reason)
{
    ...
    // ERTS_PSFLG_SIG_IN_Q can arrive late
                                          ...
                                          int is_signal)
{
    ...
    }
    {
        ...
        if (buffer->alive) {
            Uint64 nonempty_slots_before = 1;
            ...
            if (last == &first->next && !is_signal) {
                ...
                *buffer->queue.last = first;
                ...
                enqueue_signals(receiver,
                                ...
                                last_next,
                                len,
                                ERTS_PROC_GET_STATE_IF_DEBUG(receiver),
                                0,
                                0,
                                &buffer->queue,
            ...
            if ((set_nonempty && !nonempty_slots_before) || updated_erts_psflg_sig_in_q) {
                ...
                if ( updated_erts_psflg_sig_in_q ) {
                    Uint32 state = erts_atomic32_read_bor_nob(&receiver->state,
                                                              ERTS_PSFLG_SIG_IN_Q);
                    if (!(state & ERTS_PSFLG_SIG_IN_Q)) {
                        erts_atomic32_read_bor_nob(&receiver->state, ERTS_PSFLG_SIG_IN_Q);
                    }
                }
            }
            if (is_signal) {
                erts_proc_notify_new_sig(receiver, erts_atomic32_read_acqb(&receiver->state), 0);
            } else {
                if (last == &first->next) {
                    erts_proc_notify_new_message(receiver, receiver_locks);
                } else {
                    erts_proc_notify_new_sig(receiver, erts_atomic32_read_acqb(&receiver->state), ERTS_PSFLG_ACTIVE);
                }
            }
            return 1;
        }
