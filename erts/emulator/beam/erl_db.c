...
static struct {
    union {
	/*DbTable *tb; */ /* should not be used anymore */ /* Only directly readable if slot is ALIVE */
	erts_atomic_t next_free;  /* (index<<2)|1 if slot is FREE */ /* TODO: SMP only atomic */
    }u;
} *meta_main_tab;
 ...
 */
#define GET_TAB_NEXT_FREE(i) (erts_atomic_read_mb(&meta_main_tab[(i)].u.next_free))
#define GET_TAB_TABLE(i) ((DbTable*)GET_TAB_NEXT_FREE(i))
#define IS_SLOT_FREE(i)	(GET_TAB_NEXT_FREE(i) & 1)
#define IS_SLOT_DEAD(i) (GET_TAB_NEXT_FREE(i) & 2)
				     ...
				     char *rwname, char* fixname)
{
    ...
    erts_smp_atomic_init_mb(&tb->common.exclusive, (erts_aint_t)NULL);
...
static int remove_named_tab(Process* p, DbTable *tb, int have_lock)
{
    int ret = 0;
    erts_smp_rwmtx_t* rwlock;
	 ...
	 */
	db_unlock(p, tb, LCK_WRITE);
	erts_smp_rwmtx_rwlock(rwlock);
	db_lock(p, tb, LCK_WRITE);
    }
#endif
...
BIF_RETTYPE ets_rename_2(BIF_ALIST_2)
{
    ...
    if (!remove_named_tab(BIF_P, tb, 1))
...
BIF_RETTYPE ets_new_2(BIF_ALIST_2)
{
    ...
    erts_atomic_set_mb(&meta_main_tab[slot].u.next_free, (erts_aint_t)tb);
...
badarg:
    ...
    if (tb != NULL) db_unlock(BIF_P, tb, LCK_WRITE);
    BIF_ERROR(BIF_P, BADARG);
}
...
erts_db_process_exiting(Process *c_p, ErtsProcLocks c_p_locks)
{
	...
	case ErtsDbProcCleanupOpDeleteFixations:
		...
		if (tb) {
		    ...
		    db_unlock(c_p, tb, LCK_WRITE_REC);
		    BUMP_REDS(c_p, reds);
		}
		state->slots.ix++;
		if (ERTS_BIF_REDS_LEFT(c_p) <= 0)
...
unlocked:
	...
	if (*kind_p == LCK_READ && tb->common.is_thread_safe) {
	    ...
	    /* FIXME TODO XXX verify this approach in taking exclusive access is equivalent to previous locking code  */
	    db_exclusive_lock(p, tb);
	    /*
			   ...
			   int clean_meta_tab)
{
	...
	//lock_free_meta TODO have to make this thread safe!
        //mmtl = get_meta_main_tab_lock(tb->common.slot);
//#ifdef ERTS_SMP
//	if (erts_smp_rwmtx_tryrwlock(mmtl) == EBUSY) {
...
void db_info(int to, void *to_arg, int show)    /* Called by break handler */
{
	...
	if (IS_SLOT_ALIVE(i)) {
	    erts_print(to, to_arg, "=ets:%T\n", (GET_TAB_TABLE(i))->common.owner);
	    erts_print(to, to_arg, "Slot: %d\n", i);
	    print_table(to, to_arg, show, GET_TAB_TABLE(i));
	}
#ifdef DEBUG
